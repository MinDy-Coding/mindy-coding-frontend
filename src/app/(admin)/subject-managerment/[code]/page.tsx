...
import { CheckCircle, Circle, Clock, MapPin, CalendarRange, Users, FileUp, Edit, UploadCloud, Download } from "lucide-react"
...
export default function SubjectDetailPage() {
  ...
  if (!detail) {
    return (
      <div className="space-y-3">
        <Link href="/subject-managerment" className="text-sm text-blue-600 hover:underline">
          Quay lại danh sách
        </Link>
  ...
  const [tab, setTab] = useState<"thong-tin" | "lo-trinh" | "tai-lieu" | "lop-hoc">("thong-tin")

  return (
    <div className="space-y-6 relative">
      <div className="flex items-center gap-3 text-sm text-gray-600">
        <Link href="/subject-managerment" className="hover:underline">
          Quay lại danh sách
        </Link>
      </div>
      ...
      <div className="flex items-start gap-4">
        ...
        {/* edit button shown inside the detail page (top-right of header area) */}
        <button aria-label="Chỉnh sửa môn" title="Chỉnh sửa" className="absolute top-6 right-6 bg-white border border-gray-200 shadow w-10 h-10 rounded-full flex items-center justify-center">
      ...
      <div className="flex gap-1 mt-6 bg-cyan-50 w-fit rounded-full p-1">
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-medium ${
            tab === "thong-tin" ? "bg-white shadow text-cyan-700" : "text-gray-600 hover:text-black"
          }`}
        >
          Thông tin
        </button>
        <button
          onClick={() => setTab("lo-trinh")}
          className={`px-4 py-2 rounded-full text-sm font-medium ${
            tab === "lo-trinh" ? "bg-white shadow text-cyan-700" : "text-gray-600 hover:text-black"
          }`}
        >
          Lộ trình
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-medium ${
            tab === "tai-lieu" ? "bg-white shadow text-cyan-700" : "text-gray-600 hover:text-black"
          }`}
        >
          Tài liệu
        </button>
        <button
          onClick={() => setTab("lop-hoc")}
          className={`px-4 py-2 rounded-full text-sm font-medium ${
            tab === "lop-hoc" ? "bg-white shadow text-cyan-700" : "text-gray-600 hover:text-black"
          }`}
        >
          Lớp học
        </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-4">
          {tab === "thong-tin" ? (
            <Card key="thong-tin" className="border-gray-300">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg">Thông tin môn học</CardTitle>
                <p className="text-sm text-gray-500">Mô tả chi tiết về môn học</p>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <h4 className="font-semibold text-gray-900 mb-2">Mục tiêu khóa học</h4>
            ...
            <Card key="materials" className="border-gray-300 relative">
              <CardHeader className="pb-2">
                <div>
              ...
              {/* upload button positioned at top-right inside the card */}
              <div className="absolute top-3 right-3">
                <Button className="bg-cyan-100 text-cyan-700 hover:bg-cyan-200 px-4 py-2 rounded-lg flex items-center gap-2">
            ...
            <Card key="lop-hoc" className="border-gray-300">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg">Lớp học</CardTitle>
                <p className="text-sm text-gray-500">Danh sách lớp học của môn này</p>
              </CardHeader>
